 ...
 * ```
 */
function isVariableDeclaratorWithTypeAnnotation(node: TSESTree.Node): node is TSESTree.VariableDeclarator {
  return node.type === AST_NODE_TYPES.VariableDeclarator && !!node.id.typeAnnotation
}
 ...
 * ```
 */
function isPropertyDefinitionWithTypeAnnotation(node: TSESTree.Node): node is TSESTree.PropertyDefinition {
  return node.type === AST_NODE_TYPES.PropertyDefinition && !!node.typeAnnotation
}
 ...
 * ```
 */
function isFunctionArgument(parent: TSESTree.Node, callee?: FunctionExpression): parent is TSESTree.CallExpression {
  return (
 ...
 * ```
 */
function isTypedJSX(node: TSESTree.Node): node is TSESTree.JSXExpressionContainer | TSESTree.JSXSpreadAttribute {
  return node.type === AST_NODE_TYPES.JSXExpressionContainer || node.type === AST_NODE_TYPES.JSXSpreadAttribute
}

function isTypedParent(parent: TSESTree.Node, callee?: FunctionExpression): boolean {
  return (
 ...
 * ```
 */
function isPropertyOfObjectWithType(property: TSESTree.Node | undefined): boolean {
 ...
 * ```
 */
function doesImmediatelyReturnFunctionExpression({ body }: FunctionNode): boolean {
  ...
  return body.type === AST_NODE_TYPES.ArrowFunctionExpression || body.type === AST_NODE_TYPES.FunctionExpression
}
 ...
 * ```
 */
function returnsConstAssertionDirectly(node: TSESTree.ArrowFunctionExpression): boolean {
  const { body } = node
 ...
 */
function isTypedFunctionExpression(node: FunctionExpression, options: Options): boolean {
  const parent = ESLintUtils.nullThrows(node.parent, ESLintUtils.NullThrowsReasons.MissingParent)
 ...
 */
function isValidFunctionExpressionReturnType(node: FunctionExpression, options: Options): boolean {
 ...
 */
function isValidFunctionReturnType(node: FunctionNode, options: Options): boolean {
  if (options.allowHigherOrderFunctions && doesImmediatelyReturnFunctionExpression(node)) {
    return true
  }

  return node.returnType != null || isConstructor(node.parent) || isSetter(node.parent)
}
...
function ancestorHasReturnType(node: FunctionNode): boolean {
  ...
  const isBodylessArrow =
    ancestor.type === AST_NODE_TYPES.ArrowFunctionExpression && ancestor.body.type !== AST_NODE_TYPES.BlockStatement
